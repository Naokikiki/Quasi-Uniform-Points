...
> **Note on figures.** Image files (`*.png`, `*.pdf`) are git-ignored; the figures are regenerated by running the notebooks. The notebooks store their plots inline, so they render directly on GitHub.
